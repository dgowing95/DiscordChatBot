...
async def textToImage(prompt: PromptRequest):
    ...
    text_to_image_handler.release_resources()
    gc.collect()
    return StreamingResponse(io.BytesIO(image_bytes), media_type="image/png")

@app.post("/image-image")
async def imageToImage(prompt: str = Form(...), strength: float = Form(0.75), file: UploadFile = File(...)):
    ...
    image_to_image_handler.release_resources()
    gc.collect()
